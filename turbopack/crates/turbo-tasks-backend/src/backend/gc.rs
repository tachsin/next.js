...
impl TurboTasksBackend {
    ...
    pub(crate) fn gc_collect(
        ...
        // Tasks spawned for collection specifically via the edge-loss buffer (a subset of
        // `collected`) — the direct observable for the edge-loss discovery path.
        let edge_loss_collected = AtomicUsize::new(0);
        ...
        scope_unbounded(seeds, |spawner, job| {
            ...
            //
            // `Break` closes the queue: remaining jobs are discarded without being dispatched, and
            // in-flight jobs cannot re-grow it as they finish.
            ...
            for candidate in ctx.take_gc_edge_loss_candidates() {
                ...
                if ctx
                    ...
                    .is_gc_collectible()
                {
                    edge_loss_collected.fetch_add(1, Ordering::Relaxed);
        ...
        let stats = GcStats {
            ...
            edge_loss_collected: edge_loss_collected.into_inner(),
        };
