    ...
    case 'request': {
      ...
      if (staticValue !== undefined) {
        return resolveInStage(
          ...
          workUnitStore.needsSessionShell
      ...
      if (runtimeValue !== undefined) {
        // No combination fixes this one, so nothing cached may contain it,
        // not just the shells: no prerender's key mentions it. The runtime
        // stage is past every one of them, so there is no `needsSessionShell`
